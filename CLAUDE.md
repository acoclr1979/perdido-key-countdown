...
Non-pooled images stay in the repo root because they're referenced by explicit filename: `beachball.jpeg` (stories.html seed story), `blue-angels-pensacola-beach.jpg` (stories.html Blue Angels story), `california-bill.jpg` (index.html — now the "California Bill" side-quest panel, moved down from the top feature-story slot), `bushwacker.jpg` (index.html — current top feature story, the Bushwacker recipe/lore panel), `vacation-rick-depleted.png` (index.html VREI panel — payoff photo under the meter/quote box on days the score is low).

**Feature-story slot (index.html, right after the Photo Dump card):** this is the "whoever's story we're telling today" panel — currently the Bushwacker (id `bushwacker`), previously California Bill (id `california-bill`, now relocated into the Side Quest cluster near the bottom instead of deleted). It reuses the `.bw-poster-*` CSS (a warm/no-filter variant of `.bill-poster-*`, the original vintage-sepia Dos Equis-parody treatment) — pick whichever poster style fits the next story's tone rather than always defaulting to the sepia one.
