...
- `gallery.html` — "The Photo Dump," every photo in `/photos/` plus `/photos/florabama/`, `/photos/cobalt/`, and `/photos/crabtrap/`, sectioned into Beach / Flora-Bama / Cobalt / Crab Trap / Past Trips
...
- `california-bill.html` — "California Bill," side quest page: the original Dos Equis-parody "Most Interesting Man" feature, moved off index.html into its own page (still uses the sepia `.bill-poster-*` treatment, self-contained in this file's `<style>` block since nothing else references it anymore)
...
Non-pooled images stay in the repo root because they're referenced by explicit filename: `beachball.jpeg` (stories.html seed story), `blue-angels-pensacola-beach.jpg` (stories.html Blue Angels story), `california-bill.jpg` (california-bill.html feature), `bushwacker.jpg` (index.html — current top feature story, the Bushwacker recipe/lore panel), `vacation-rick-depleted.png` (index.html VREI panel — payoff photo under the meter/quote box on days the score is low).

**Feature-story slot (index.html, right after the Photo Dump card):** this is the "whoever's story we're telling today" panel — currently the Bushwacker (id `bushwacker`). It uses the `.bw-poster-*` CSS (a warm/no-filter variant of the `.bill-poster-*` sepia Dos Equis-parody treatment now living in `california-bill.html`) — pick whichever poster style fits the next story's tone. When a feature story gets swapped out, follow the California Bill precedent: give it its own page (mirroring `florabama.html`/`cobalt.html`) and collapse it into a slim `Side Quest` link card in the cluster near the bottom of `index.html`, rather than deleting it or leaving it as a full inline panel.
...
**Crab Trap photo section (gallery.html only):** `photos/crabtrap/` is a subfolder feeding a dedicated "🦀 Crab Trap" section on `gallery.html` (unlike Flora-Bama/Cobalt, there's no standalone `crabtrap.html` page). `gallery.js` loads it via the shared `pkcLoadSubfolderPhotos('photos/crabtrap', 'pkc_crabtrap_photos_cache_v1')` helper, same pattern as the other two subfolders. Drop photos in via GitHub's web/mobile upload UI or `git add`/commit/push — no filename convention needed. A `README.md` placeholder keeps the empty folder tracked in git until photos land.
...
- `photos/Photos-37-PKC.jpeg` — current index.html hero photo (whole-group shot at Flora-Bama Yacht Club, dusk, red sail shade) and `og:image`. It's a copy of `photos/florabama/Photos-37-PKC.jpeg` (same filename, both copies kept so it shows in both the main pool and the Flora-Bama collage) — filename has the plural "Photos-" typo from the original upload, left as-is since it's already referenced by exact name in `index.html`. Replaced the Photo-35 two-fisted-drinks shot; Photo-35 stays in the regular photo pool.
